...
public class ScreenRecorder extends Activity {
    ...
    public static void startScreenRecorder(int w, int h, int d, int bit) {
        width = w;
        height = h;
        dpi = d;
        bitRate = bit;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 5) {
            if (resultCode == RESULT_OK) {
                ...
                virtualDisplay = mMediaProjection.createVirtualDisplay(TAG,width,height,dpi, DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR,surface,null,null);
